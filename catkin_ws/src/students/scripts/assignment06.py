...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for [d,theta] in laser_readings:
    	mag = 0 if d >= d0 or d<= 0: else: mag = eta*math.sqrt(1/d - 1/d0)
    	force_x += math.cos(robot_a + theta)
    	force_y += math.sin(robot_a + theta)
...
def callback_pot_fields_goal(msg):
    ...
    while distance_to_goal_point > tolerance and not rospy.is_shutdown():
        ...
        [frx,fry] = rejection_force(robot_x, robot_y, laser_readings)

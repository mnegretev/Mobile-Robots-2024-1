...
#define FULL_NAME "JARQUIN LOPEZ DANIEL EDUARDO"
...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        particles.poses[i].position.z = 0.0; 

        particles.poses[i].orientation.x = 0.0;
        particles.poses[i].orientation.y = 0.0;
        particles.poses[i].orientation.z = sin_half_theta;
        particles.poses[i].orientation.w = cos_half_theta;
    }
...
int random_choice(std::vector<float>& similarities)
{
    ...
    float random_value = rnd.uniformReal(0.0, total_similarity);

    float cumulative_probability = 0.0;
    for (size_t i = 0; i < similarities.size(); ++i)
    {
        cumulative_probability += similarities[i];

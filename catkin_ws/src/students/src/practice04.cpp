...
#define FULL_NAME "FELIPE RODRIGUEZ FUENTES"
...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
     ...
     */
    for(size_t 1 = 0; i<particles.poses.size(); i++)
    {
    ...
    for(size_t i=0; i<particles.poses.size(); i++)
     {
         ...
         resampled_particles.poses[i].position.x = particle.poses[idx].position.x + rnd.gaussian(0, RESAMPLING_NOISE);
         resampled_particles.poses[i].position.y = particle.poses[idx].position.y + rnd.gaussian(0, RESAMPLING_NOISE);
